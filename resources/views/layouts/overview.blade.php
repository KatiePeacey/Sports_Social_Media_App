<div class="bg-white py-24 sm:py-32">
  <div class="mx-auto grid max-w-7xl gap-20 px-6 lg:px-8 xl:grid-cols-3">
    ...
    <ul role="list" class="grid gap-x-8 gap-y-12 sm:grid-cols-2 sm:gap-y-16 xl:col-span-2">
      ...
      <li>
            <h3 class="text-base/7 font-semibold tracking-tight text-gray-900">Name: <a href="{{ route('posts.show', $post) }}">{{ $post->player->name }}</a></h3>
            <p class="text-sm/6 font-semibold text-indigo-600">Content: {{ $post->content }}</p>
      </li>
        @endforeach
      <!-- More people... -->
    </ul>
